...
p = Path('ctrader/NAI-MOUNTAIN-STORY-ENGINE-V2/NAI-MOUNTAIN-STORY-ENGINE-V2.cs')
s = p.read_text()
...
once('        private string _requestedCloseReason = string.Empty;\n', '''        private string _requestedCloseReason = string.Empty;
...
once('                double breakBuffer = Threshold * 0.15;\n', '                double breakBuffer = Threshold * 0.28;\n', 'transition buffer')

once('''            _microThreshold = Math.Max(spread * 0.90, noise * 4.0);
...
once('''            Position position = GetPosition();
            if (position != null)
            {
                _hadPosition = true;
...
once('            double buffer = Math.Max(CurrentSpread() * 0.08, _noiseEma * 0.50);\n', '            double buffer = Math.Max(CurrentSpread() * 0.05, _noiseEma * 0.30);\n', 'pullback buffer')

once('''        private bool FastFollowThrough(Direction direction)
        {
            if (_ticks.Count < 13)
                return false;
...
once('''            double stopBuffer = Math.Max(spread * 0.30, _microThreshold * 0.25);
...
''', 'active-structure entry stop')

once('''            _lastTradedPullbackSerial = _pullbackSerial;
            _hadPosition = true;
...
once('''        private bool CheckVirtualStopFirst(Position position, double bid, double ask)
        {
...
once('''        private void ManageOpenTrade(Position position, double bid, double ask, double mid)
        {
...
once('''        private bool ThesisBroken(Position position, double mid, bool trendlineBreached)
        {
            Direction tradeDirection = position.TradeType == TradeType.Buy ? Direction.Up : Direction.Down;
            Direction opposite = Opposite(tradeDirection);
...
for old, new in [
    ('healthFactor = 1.12;', 'healthFactor = 1.35;'),
    ('healthFactor = 1.00;', 'healthFactor = 1.20;'),
    ('healthFactor = 0.82;', 'healthFactor = 1.00;'),
    ('healthFactor = 0.62;', 'healthFactor = 0.85;'),
    ('targetFactor = 0.58;', 'targetFactor = 0.80;'),
    ('targetFactor = 0.68;', 'targetFactor = 0.88;'),
    ('targetFactor = 0.84;', 'targetFactor = 0.95;'),
    ('phaseFactor = 0.82;', 'phaseFactor = 0.95;'),
    ('phaseFactor = 0.62;', 'phaseFactor = 0.85;'),
    ('phaseFactor = 0.52;', 'phaseFactor = 0.78;'),
    ('double lineFactor = trendlineBreached ? 0.72 : 1.0;', 'double lineFactor = trendlineBreached ? 0.90 : 1.0;'),
    ('double tinyProfitActivation = Math.Max(spread * 1.35, _trade.TargetBenchmark * 0.35);', 'double tinyProfitActivation = Math.Max(spread * 1.60, _trade.TargetBenchmark * 0.60);')
]:
    once(old, new, 'trail tuning ' + old)

once('''        private double StructureTrailCandidate(Position position)
        {
...
''', 'active-only trail structure')

once('''                    _hadPosition = false;
                    _trade.Clear();
''', '''                    _hadPosition = false;
                    _trade.Clear();
...
once('''            _trade.EntryProtectedPrice = _active.ProtectedPrice;
            _trade.EntryTrailBenchmark = _trailBenchmark;
        }

        private double ComputeTradeHealth''', '''            _trade.EntryProtectedPrice = _active.ProtectedPrice;
            _trade.EntryTrailBenchmark = _trailBenchmark;
            ResetHoldState(true);
        }

        private double ComputeTradeHealth''', 'reattach hold')

once('            Print("NAI MOUNTAIN STORY ENGINE V2 STARTED | DEMO ONLY");', '            Print("NAI MOUNTAIN STORY ENGINE V2 + STICKY HOLD V3 STARTED | DEMO ONLY");\n            Print("LAW | COME EARLIER. HOLD MICRO PULLBACKS. EXIT ONLY AFTER 5-TICK STRUCTURAL CONFIRMATION.");', 'startup')

p.write_text(s)
print('SMFN Sticky Hold V3 patch applied successfully')
