...
// VWAP entry identity remains the same: depart -> retest -> reclaim.
// V5.33 adds an M5-sized minimum departure so tiny VWAP vibration is not
// promoted into a fresh setup merely because M1 volatility is quiet.
input double DepartureAdaptiveFrac=0.60;
input double MinVWAPDepartureM5RangeFrac=0.50;
...
// V5.33 mountain breathing room.
// A shallow EMA violation must persist for 3 completed M5 closes.
// A one-close emergency exit needs a much deeper 0.60 x M5 loss of EMA8.
input int EMAExitWrongCloses=3;
input double EMAClearBreakM5RangeFrac=0.60;

// Preserved from V5.32. Guard is active only before Mountain Lock.
...
double youngMAE=0;
int zombieCuts=0;
int m5DepartureBlocks=0;

void Log(string s){ if(VerboseLog) Print("[V5.33-MH] ",s); }
...
bool OpenBuy()
{
   ...
   bool ok=trade.Buy(lots,_Symbol,0,sl,0,"V533_MH_BUY");if(!ok||!TradeOK()){Log("BUY failed: "+trade.ResultRetcodeDescription());return false;}ResetSignal();ResetRunner();lastAction="BUY OPENED";Log(lastAction+" | young phase started");return true;
}
...
bool OpenSell()
{
   ...
   bool ok=trade.Sell(lots,_Symbol,0,sl,0,"V533_MH_SELL");if(!ok||!TradeOK()){Log("SELL failed: "+trade.ResultRetcodeDescription());return false;}ResetSignal();ResetRunner();lastAction="SELL OPENED";Log(lastAction+" | young phase started");return true;
}
...
void ReviewEMA()
{
   ...
   if(ty==POSITION_TYPE_BUY){wrong=b.close<ema;if(wrong)depth=ema-b.close;}
   else{wrong=b.close>ema;if(wrong)depth=b.close-ema;}
   if(!wrong){emaWrongCloses=0;runnerHealth="EMA8 RESPECTED - HOLD MOUNTAIN";return;}

   emaWrongCloses++;
   double frac=(avgM5>0?depth/avgM5:0);
   bool clearBreak=frac>=MathMax(0.0,EMAClearBreakM5RangeFrac);
   bool repeated=emaWrongCloses>=MathMax(1,EMAExitWrongCloses);

   if(clearBreak||repeated){
      string why=clearBreak?
         "EMA8 EXIT - DEEP "+DoubleToString(EMAClearBreakM5RangeFrac,2)+"x M5 LOSS":
         "EMA8 EXIT - "+IntegerToString(EMAExitWrongCloses)+" WRONG M5 CLOSES";
      if(ClosePos(tk,why)){lastAction=why+" | FLAT";Log("MATURE EXIT -> FLAT, WAIT FRESH VWAP");}
      return;
   }
...
void ProcessSignal(const MqlTick &t)
{
   ...
   if(state==IDLE){
      if(allowBuy&&price>=vwap+departure){state=BUY_DEP;lastAction="BUY REAL VWAP DEPARTURE | "+DoubleToString((avgM5>0?(price-vwap)/avgM5:0),2)+" M5R";return;}
      if(allowSell&&price<=vwap-departure){state=SELL_DEP;lastAction="SELL REAL VWAP DEPARTURE | "+DoubleToString((avgM5>0?(vwap-price)/avgM5:0),2)+" M5R";return;}
      return;
   }

   if((state==BUY_DEP||state==BUY_RET)&&!allowBuy){ResetSignal();return;}
   if((state==SELL_DEP||state==SELL_RET)&&!allowSell){ResetSignal();return;}
   if(state==BUY_DEP){if(price<=vwap+band){state=BUY_RET;retestStart=TimeCurrent();retestExtreme=price;wrongSideSince=0;lastAction="BUY VWAP RETEST AFTER REAL DEPARTURE";}return;}
   if(state==SELL_DEP){if(price>=vwap-band){state=SELL_RET;retestStart=TimeCurrent();retestExtreme=price;wrongSideSince=0;lastAction="SELL VWAP RETEST AFTER REAL DEPARTURE";}return;}
   if(state==BUY_RET){
      retestExtreme=MathMin(retestExtreme,price);
      if(RetestTimeoutMinutes>0&&retestStart>0&&TimeCurrent()-retestStart>RetestTimeoutMinutes*60){ResetSignal();return;}
      if(price<vwap-wrongDepth){if(wrongSideSince==0)wrongSideSince=TimeCurrent();if(TimeCurrent()-wrongSideSince>=MathMax(1,VWAPWrongSideSeconds)){ResetSignal();return;}}
      else if(price>=vwap)wrongSideSince=0;
      bool reclaimed=price>=vwap+reclaim,bounced=price>=retestExtreme+reclaim;if(reclaimed&&bounced)ExecuteSignal(SIG_BUY);return;
   }
   if(state==SELL_RET){
      ...
      bool reclaimed=price<=vwap-reclaim,dropped=price<=retestExtreme-reclaim;if(reclaimed&&dropped)ExecuteSignal(SIG_SELL);return;
   }
}
...
void Panel(const MqlTick &t)
{
   ...
   double minDep=(avgM5>0?avgM5*MinVWAPDepartureM5RangeFrac:0);
   Comment("VWAP + EMA8 MOUNTAIN RUNNER v5.33 MOUNTAIN HOLD\n",
           "POSITION: ",pos,"\nSTATE: ",StateName(),"\nVWAP: ",(haveVWAP?DoubleToString(liveVWAP,_Digits):"n/a"),
           "\nM5 AVG RANGE: ",DoubleToString(avgM5,2)," | MIN VWAP DEP: ",DoubleToString(minDep,2)," (",DoubleToString(MinVWAPDepartureM5RangeFrac,2),"R)",
...
int OnInit()
{
   ...
   Log("INIT V5.33 MOUNTAIN HOLD | real M5-sized VWAP departure + wider EMA breathing + V5.32 zombie guard");
   Log("ENTRY DEFAULT | VWAP departure >= max(adaptive, 0.50x M5, cost floor)");
   Log("MATURE DEFAULT | 3 wrong M5 closes OR deep 0.60x M5 EMA loss");
   return INIT_SUCCEEDED;
}

void OnDeinit(const int reason)
{
   Log("DEINIT V5.33 | zombie cuts="+IntegerToString(zombieCuts));if(emaHandle!=INVALID_HANDLE)IndicatorRelease(emaHandle);Comment("");
}

void OnTick()
{
   MqlTick t;if(!SymbolInfoTick(_Symbol,t))return;
   RecordTick(t);NewM1();NewM5();if(!UpdateVWAP())return;UpdateRunner(t);
