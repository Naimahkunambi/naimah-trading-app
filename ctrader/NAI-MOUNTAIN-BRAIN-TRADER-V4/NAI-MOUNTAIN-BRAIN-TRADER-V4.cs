...
namespace cAlgo.Robots
{
    ...
    public class NaiMountainBrainTraderV4 : Robot
    {
        private const string Label = "NAI-MOUNTAIN-BRAIN-TRADER-V4";
        ...
        protected override void OnStart()
        {
            if (Account.IsLive)
            {
                Print("V4 SAFETY | LIVE ACCOUNT DETECTED | STOPPING. DEMO ONLY.");
                Stop();
                return;
            }
            ...
            Print("NAI MOUNTAIN BRAIN TRADER V4 STARTED | DEMO ONLY");
            Print("V4 LAW 1 | M5 is context, never entry permission by itself");
            Print("V4 LAW 2 | no waterfall chasing: pullback must be meaningful relative to impulse");
            Print("V4 LAW 3 | preserve small wins; wrong thesis exits early instead of holding to full stop");
        }
        ...
        protected override void OnTick()
        {
            ...
            if (_hadPosition)
            {
                Print("V4 RESET | prior trade closed | old setup retired | fresh impulse + meaningful pullback required");
                _hadPosition = false;
            ...
            if (newContext != _contextSide)
            {
                ...
                Print("V4 CONTEXT | {0} -> {1} | local cycle reset", oldText, newText);
        ...
        private void TryEnter(TradeType side, double mid)
        {
            ...
            {
                Print("V4 NO CHASE | cycle={0} {1} | pullback only {2:P0} of impulse | waterfall / tiny reset rejected", _cycleId, side, pullbackRatio);
            ...
            {
                Print("V4 NO ENTRY | cycle={0} {1} | pullback {2:P0} of impulse is too deep | old context may be failing", _cycleId, side, pullbackRatio);
                BeginFreshCycle(mid, side);
                return;
            }
            ...
            {
                if (JournalDetail)
                    Print("V4 WAIT | context=SELL but LOCAL M1=UP | context is not permission | waiting for local buy to fail");
                return;
            }
            if (side == TradeType.Buy && localBias == LocalBias.Down)
            {
                if (JournalDetail)
                    Print("V4 WAIT | context=BUY but LOCAL M1=DOWN | context is not permission | waiting for local sell to fail");
                return;
            }
            ...
            {
                if (JournalDetail)
                    Print("V4 BREAK WAIT | cycle={0} {1} | aligned={2}/5 short={3:F2} medium={4:F2} | structure broke but follow-through not ready", _cycleId, side, aligned, shortMove, mediumMove);
                return;
            }
        ...
        private void OpenTrade(TradeType side, double mid, double pullbackRatio, LocalBias localBias, int aligned, double shortMove, double mediumMove)
        {
            ...
            {
                Print("V4 ENTRY SKIP | volume {0} below minimum {1}", volume, Symbol.VolumeInUnitsMin);
                BeginFreshCycle(mid, side);
                return;
            }
            ...
            Print("V4 THESIS | {0} | M5 context only | local pullback failed | pb/imp={1:P0} localM1={2} aligned={3}/5 short={4:F2} medium={5:F2} | target={6:F2}R plannedRisk=${7:F2}",
                side, pullbackRatio, localBias, aligned, shortMove, mediumMove, TargetR, riskAmount);

            TradeResult result = ExecuteMarketOrder(side, SymbolName, volume, Label, slPips, tpPips);
            if (!result.IsSuccessful || result.Position == null)
            {
                Print("V4 ENTRY REJECTED | error={0}", result.Error);
                BeginFreshCycle(mid, side);
                return;
            }
            ...
            Print("V4 ENTER {0} | id={1} entry={2:F2} SL={3} TP={4} volume={5} | small-win mode",
                side,
                p.Id,
                p.EntryPrice,
        ...
        private void ManageOpenPosition(Position p)
        {
            ...
            {
                Print("V4 HARD LOSS CAP | id={0} net={1:F2} cap={2:F2} | closing", p.Id, p.NetProfit, hardLossCap);
                ClosePosition(p);
                return;
            }

            // Thesis is supposed to resume quickly after the local structure break.
            // If it never showed follow-through, do not hold the wrong side to the structural stop.
            if (_highestR < 0.10 && r <= -EarlyFailureR)
            {
                Print("V4 EARLY FAILURE EXIT | id={0} r={1:F2} bestR={2:F2} | no follow-through after entry", p.Id, r, _highestR);
                ClosePosition(p);
                return;
            }
            ...
            {
                Print("V4 STALE THESIS EXIT | id={0} age={1:F0}s r={2:F2} bestR={3:F2} | trade never proved itself", p.Id, seconds, r, _highestR);
                ClosePosition(p);
                return;
            }
            ...
            {
                Print("V4 LOCAL INVALIDATION EXIT | id={0} local={1} r={2:F2} bestR={3:F2}", p.Id, liveLocal, r, _highestR);
                ClosePosition(p);
                return;
            }
            ...
            Print("V4 PROTECT REQUEST | id={0} r={1:F2} bestR={2:F2} lock={3:F2}R oldSL={4} newSL={5:F2}",
                p.Id, r, _highestR, lockR.Value, p.StopLoss.HasValue ? p.StopLoss.Value.ToString("F2") : "NONE", desired);

            TradeResult result = ModifyPosition(p, desired, p.TakeProfit);
            if (result.IsSuccessful)
            {
                Print("V4 PROTECT SUCCESS | id={0} newSL={1:F2}", p.Id, desired);
                return;
            }

            Print("V4 PROTECT REJECTED | id={0} error={1} | closing rather than leaving an expensive exposed winner", p.Id, result.Error);
            ClosePosition(p);
        }
        ...
        private void BeginFreshCycle(double mid, TradeType? side)
        {
            ...
            if (side.HasValue && JournalDetail)
                Print("V4 CYCLE #{0} | {1} context | SEEKING FRESH IMPULSE", _cycleId, side.Value);
        }

        private void RejectAndRestart(TradeType side, double mid, string reason)
        {
            Print("V4 CYCLE REJECTED | cycle={0} side={1} reason={2}", _cycleId, side, reason);
            BeginFreshCycle(mid, side);
        }
        ...
        private void PrintStatus(double mid)
        {
            ...
            Print("V4 STATUS | M5={0} localM1={1} state={2} cycle={3} pb/imp={4:P0} mid={5:F2} noise={6:F2}",
        ...
        private void LogState(string name, TradeType side, double mid, double observed, double needed)
        {
            ...
            double ratio = _impulseDistance > Symbol.TickSize ? _pullbackDistance / _impulseDistance : 0;
            Print("V4 CYCLE #{0} | {1} | {2} | mid={3:F2} observed={4:F2} need={5:F2} pb/imp={6:P0}",
        ...
        {
            if (JournalDetail)
                Print("V4 CYCLE #{0} | {1}", _cycleId, text);
        }
    }
}
