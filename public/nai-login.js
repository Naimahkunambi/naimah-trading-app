...
async function begin(){
  ...
  const authUrl=`${AUTH}?${q.toString()}`;
  const a=$('continueDeriv');
  a.href=authUrl;a.classList.add('show');
  status('Deriv login is prepared. Click CONTINUE TO DERIV below.');
}
$('login').onclick=begin;

async function exchange(){
  const q=new URLSearchParams(location.search);
  if(q.get('error')){status(`Deriv login failed: ${q.get('error_description')||q.get('error')}`);history.replaceState({},'',location.pathname);return false}
  const code=q.get('code');if(!code)return false;
  const returnedState=q.get('state'),expected=sessionStorage.getItem(SS.s),verifier=sessionStorage.getItem(SS.v),client=sessionStorage.getItem(SS.c);
  if(!returnedState||!expected||returnedState!==expected){sessionStorage.removeItem(SS.v);sessionStorage.removeItem(SS.s);history.replaceState({},'',location.pathname);status('OAuth session did not match. Press PREPARE DERIV LOGIN again.');return false}
  if(!verifier||!client){history.replaceState({},'',location.pathname);status('OAuth session expired. Prepare login again.');return false}
  ...
  if(!r.ok||!j.accessToken){history.replaceState({},'',location.pathname);status(`OAuth error: ${j.error||r.status}`);return false}
  sessionStorage.setItem(SS.t,j.accessToken);sessionStorage.setItem(SS.exp,String(Date.now()+Number(j.expiresIn||3600)*1000));sessionStorage.removeItem(SS.v);sessionStorage.removeItem(SS.s);history.replaceState({},'',location.pathname);return true
}
