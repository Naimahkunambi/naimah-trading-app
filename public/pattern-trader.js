...
const VERSION = 'v6.9';
const LEDGER_KEY = 'sani.masterTrader.signalLedger.v6.9';
const VISUAL_KEY = 'sani.masterTrader.visualSetups.v6.9';
...
function saveVisuals() {
  visualHistory = visualHistory.slice(0, 1600);
...
function structure(p, radius = 3) {
  ...
  const highs = x.highs.slice(-2), lows = x.lows.slice(-2);
...
function liveWave(rows, direction) {
  ...
  if (direction === 'BULL') {
    for (const low of [...x.lows].reverse()) {
      ...
      const high = Math.max(...future);
      const i = low.i + 1 + future.indexOf(high);
    ...
    for (const high of [...x.highs].reverse()) {
      ...
      if (future.length < 3) continue;
      const low = Math.min(...future);
      const i = high.i + 1 + future.indexOf(low);
...
function waveKey(wave, rows) {
  return wave ? `${strategy.sessionId}:${wave.direction}:${rows[wave.start.i]?.epoch}` : '';
}
...
function microResumption(rows, direction, step) {
  ...
  if (n < 9 || (direction !== 'BULL' && direction !== 'BEAR')) {
    return { ok: false, kind: 'SEARCH', level: NaN, strength: 0, holdTicks: 0 };
  }
...
function reanchor(all, direction) {
  for (const width of [48, 38, 30, 24, 20]) {
    ...
    const wave = liveWave(rows, direction);
...
function rememberVisual(d) {
  ...
  const memory = waveMemory.get(d.waveKey) || { armed: false, terminal: false, signaled: false, dirBlocked: false, breakSeen: false };
  ...
  if (memory.armed && d.trigger?.holdTicks === 1 && !memory.breakSeen) {
    ...
    pushVisual(d, 'BREAK', `Micro break printed, waiting hold`);
  }
  if (memory.armed && !memory.terminal && d.phase === 'WAIT_POCKET') {
    memory.terminal = true;
    pushVisual(d, 'MISSED', 'Left pocket before confirmed micro resumption');
  }
...
function evaluate(snapshot) {
  ...
  else if (phase === 'BREAK_WAIT_HOLD') d.reason = `MICRO BREAK · waiting one-tick hold above/below ${trigger.level.toFixed(2)} · ${duration}t`;
...
function ensureRow(signal) {
  ...
  if (row) return row;
  row = {
    id: `mt69-${signal.epoch}-${Date.now()}`,
    cohort: 'v6.9-confirmed-resumption-duration',
    signalKey: key,
    observedAt: Date.now(),
    ...signal,
...
function auth() {
  ...
  if (String(selectedAccount.account_type).toLowerCase() === 'real') throw new Error('Master v6.9 is Demo-only.');
...
function maybeTrade(snapshot) {
  ...
  try {
    ...
    engine.execute({
      ...
      structure: `master-v6.9-break-hold-${signal.duration}t`,
      epoch: signal.epoch,
      quote: signal.quote,
    ...
    engine.log('success', `MASTER v6.9 ${signal.session} ${signal.direction} · BREAK+HOLD ${signal.fibEntryRetrace.toFixed(3)} · ${signal.duration}t · 80 ${signal.slope80.toFixed(3)} · 200 ${signal.slope200.toFixed(3)}`);
  } catch (error) {
...
function renderState(d) {
  ...
  if (!d || !d.ready) {
    ...
    $('ptSignal').innerHTML = `<b class="${d.session === 'BULL' ? 'positive' : 'negative'}">CONFIRMED ${d.session === 'BULL' ? 'CALL' : 'PUT'} · ${d.duration}T</b><span>Micro structure broke and held · retrace ${(d.wave.currentRetrace * 100).toFixed(1)}% · expected entry T+${offsetEstimate()}</span>`;
  }
}
...
function drawMarker(ctx, marker, x, y) {
  ...
  } else if (marker.state === 'BREAK') {
    ctx.fillStyle = '#7fc8ff';
    ctx.font = '10px system-ui';
    ctx.fillText('B?', x + 4, y - 5);
    ctx.beginPath(); ctx.arc(x, y, 3, 0, Math.PI * 2); ctx.fill();
  } else if (marker.state === 'MISSED' || marker.state === 'INVALID') {
...
function draw() {
  ...
  if (d?.wave && d?.waveRows) {
    ...
    if (Number.isFinite(startEpoch) && Number.isFinite(endEpoch)) {
      const sx = xFor(startEpoch), ex = xFor(endEpoch);
      const sy = yFor(d.wave.start.quote), ey = yFor(d.wave.end.quote);
      ...
      for (const [ratio, label] of [[FIRE_MIN,'FIRE 32'],[.382,'38.2'],[.5,'50'],[.618,'61.8'],[FIRE_MAX,'FIRE 68'],[.786,'78.6 INVALID']]) {
        ...
        ctx.font = '10px system-ui';
        ctx.fillText(label + (label.includes('FIRE') || label.includes('INVALID') ? '' : '%'), w - 82, y - 3);
      }
    }
  }
  ...
  if (Number.isFinite(d?.trigger?.level)) {
    ...
    ctx.setLineDash([5, 4]);
    ctx.beginPath(); ctx.moveTo(w * .72, y); ctx.lineTo(w - 12, y); ctx.stroke(); ctx.setLineDash([]);
    ctx.fillStyle = '#7fc8ff';
    ctx.font = '10px system-ui';
  ...
  for (const row of ledger.filter(r => Number.isFinite(+r.contractId) && Number(r.entryTickTime ?? r.epoch) >= visibleStart && Number(r.entryTickTime ?? r.epoch) <= visibleEnd)) {
    const sx = xFor(Number(row.signalEpoch ?? row.epoch));
    const sy = yFor(Number(row.signalQuote ?? row.quote));
    ctx.fillStyle = row.direction === 'CALL' ? '#67d99a' : '#ff7474';
    ctx.font = '10px system-ui';
    ctx.fillText(`S${row.duration || '?'}`, sx + 5, sy - 7);

    const entryEpoch = Number(row.entryTickTime ?? row.epoch);
    const entryPrice = Number(row.entrySpot ?? row.quote);
    const x = xFor(entryEpoch), y = yFor(entryPrice);
    ...
    if (Number.isFinite(exitEpoch) && Number.isFinite(exitPrice) && exitEpoch >= visibleStart && exitEpoch <= visibleEnd) {
      const ex = xFor(exitEpoch), ey = yFor(exitPrice);
      ...
      ctx.fillText(`X${row.duration || '?'} ${Number(row.profit || 0) >= 0 ? '+' : ''}${Number(row.profit || 0).toFixed(2)}`, ex + 7, ey - 7);
    }
  }

  $('masterCanvasCaption').textContent = `${session} · ${d?.dirLock?.ok ? 'DIR LOCKED' : 'DIR BLOCK'} · ${d?.duration || '—'}T · ${d?.phase || 'SEARCHING'} · blue=micro break level · B?=break awaiting hold · S=confirmed signal · E=Deriv entry · X=expiry · ${rows.length} ticks`;
}
...
$('ptStart').onclick = () => {
  ...
  try {
    auth();
    traderConfig();
    if (bought() >= +($('ptMaxTrades').value || 100)) throw new Error('v6.9 cohort cap reached.');
    engine.start();
    engine.log('info', 'Master v6.9 armed: direction lock → Fib pocket → micro structure break → one-tick hold → 5t/8t expiry split.');
  } catch (error) {
    showError(error.message);
  }
};
...
$('ptClearLedger').onclick = () => {
  if (confirm('Clear fresh v6.9 confirmed-resumption cohort and visual trail?')) {
    ledger = [];
    visualHistory = [];
    waveMemory.clear();
...
engine.subscribe(state => {
  ...
  $('ptTradeRows').innerHTML = state.trades.length ? state.trades.map(trade => {
    ...
    return `<tr><td>#${trade.contractId}</td><td>${esc(`${meta.session || '—'} · BREAK+HOLD`)}</td><td>${trade.direction}</td><td>${esc(`${trade.duration || meta.duration || '—'}T · ${meta.triggerKind || '—'} · ${(Number(meta.fibEntryRetrace || 0) * 100).toFixed(0)}% · Q${meta.quality ?? '—'}`)}</td><td><span class="result ${trade.status}">${trade.status}</span></td><td>${trade.duration}t</td><td>${expected}</td><td>${actual}</td><td>${trade.latencyClass || '—'}</td><td class="${(trade.profit ?? 0) >= 0 ? 'positive' : 'negative'}">${trade.profit === undefined ? '—' : `${trade.profit >= 0 ? '+' : ''}${Number(trade.profit).toFixed(2)}`}</td><td>${trade.sendToAckMs === undefined ? '—' : Number(trade.sendToAckMs).toFixed(0) + 'ms'}</td><td>S ${meta.signalQuote ?? meta.quote ?? '—'} → E ${trade.entrySpot ?? '—'} → X ${trade.exitSpot ?? '—'}${row && Number.isFinite(+row.mfe) ? ` · MFE ${(+row.mfe).toFixed(1)}/MAE ${(+row.mae).toFixed(1)}` : ''}</td></tr>`;
  }).join('') : '<tr><td colspan="12" class="empty">No v6.9 trades yet.</td></tr>';

  if (state.logs?.[0]) {
    $('ptLogs').innerHTML = state.logs.slice(0, 70).map(log => `<div class="log ${log.level}"><time>${new Date(log.at).toLocaleTimeString()}</time><span>${esc(log.message === 'Engine armed. Waiting for fresh BOS.' ? 'Master v6.9 execution engine armed.' : log.message)}</span></div>`).join('');
  }
  renderLedger();
  draw();
});
...
window.addEventListener('DOMContentLoaded', () => {
  document.querySelector('.topbar h1')?.replaceChildren(document.createTextNode('Master Regime Trader v6.9'));
  const title = [...document.querySelectorAll('.sectionTitle span')].find(x => x.textContent.includes('Master Trader v6'));
  if (title) title.textContent = 'Master Trader v6.9 · Confirmed Resumption + 5T/8T Lab';
  const rules = [...document.querySelectorAll('.sectionTitle span')].find(x => x.textContent.includes('Frozen v6'));
  if (rules) rules.textContent = 'Frozen v6.9 break-hold / 5t–8t rules';
  if ($('ptStart')) $('ptStart').textContent = 'Start Master Trader v6.9';
  if ($('ptCooldown')) $('ptCooldown').value = '0';

  const labels = [...document.querySelectorAll('.label, .statLabel, .metricLabel, small')];
  const fiveLabel = labels.find(x => x.textContent.trim() === 'BULL W/L' || x.textContent.includes('3T W/L') || x.textContent.includes('A FIRST-TURN'));
  if (fiveLabel) fiveLabel.textContent = '5T W/L · P/L';
  const eightLabel = labels.find(x => x.textContent.trim() === 'BEAR W/L' || x.textContent.includes('5T W/L') || x.textContent.includes('B PRE-TURN'));
  if (eightLabel && eightLabel !== fiveLabel) eightLabel.textContent = '8T W/L · P/L';

  const fixedText = [...document.querySelectorAll('.muted')].find(x => x.textContent.includes('v6.8') || x.textContent.includes('v6.7') || x.textContent.includes('Fixed internals:'));
  if (fixedText) fixedText.textContent = 'v6.9: direction must remain coherent, retrace must enter the same 32–68% fire pocket, then the 20-tick micro structure must break in trend direction and survive one more tick before entry. No first-turn guessing. Qualified waves are deterministically split 5t vs 8t so expiry is the controlled exit test. Demo only, one-open lock, zero default cooldown.';
  ...
  if (stateTitle) stateTitle.textContent = 'v6.9 confirmed-resumption state machine';
