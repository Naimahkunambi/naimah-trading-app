...
const STORAGE_KEY='sani.libra.forward-timing.v2';
const PAYOUT=.92;
...
function fresh(){return{version:2,offsetTicks:1,preArmConfirm:PREARM_CONFIRM,passes:bucket(),rejects:bucket(),lastDecision:null,lastLesson:'Forward timing v2: PRE-ARM is the only paid timing path. Fully confirmed entries stay shadow-only until paid evidence proves they are not late.',paidAckMs:[],paidSlips:[],latencyHoldUntil:0,lastAckMs:0}}
function load(){try{const raw=JSON.parse(localStorage.getItem(STORAGE_KEY)||localStorage.getItem('sani.libra.forward-timing.v1')||'null');const f=fresh();return raw&&typeof raw==='object'?{...f,...raw,version:2,passes:{...f.passes,...(raw.passes||{}),recent:Array.isArray(raw.passes?.recent)?raw.passes.recent.slice(-240):[]},rejects:{...f.rejects,...(raw.rejects||{}),recent:Array.isArray(raw.rejects?.recent)?raw.rejects.recent.slice(-240):[]}}:f}catch{return fresh()}}
const state=load();
...
function evaluateForward(signal,m=mountain){
  ...
  // We target the contract we can actually obtain: decision on T, expected entry around T+1, settlement T+2.
  // The paid trigger is intentionally BEFORE full pullback confirmation.
  if(m.entryMode==='WAIT_PULLBACK_END'&&Number(m.confirmation||0)>=PREARM_CONFIRM){
    return{allowed:true,action:'PRE-ARM PASS',direction,reason:`ONE-TICK LEAD: ${m.direction} mountain intact, pullback turn ${m.confirmation}/6. SANI predicts T+1→T+2, so send before full confirmation.`,mountain:{...m,entryMode:'PRE_PULLBACK_END'},forwardOffset:1,preArm:true};
  }

  // The current audit showed confirmed TAUGHT PASS entries losing badly. Keep them as classroom evidence only.
  if(m.entryMode==='PULLBACK_END')return{allowed:false,action:regime.startsWith('DRIVE')?'LATE DRIVE':'CONFIRMED TOO LATE',direction,reason:`${regime}: the pullback is already fully confirmed. For the 1-tick paid contract this is now shadow-only; paid entry needed PRE_PULLBACK_END one tick earlier.`,mountain:m,forwardOffset:1};
...
function install(){
  ...
  window.LIBRA_TEACHER.decisionFor=(signal)=>{
    ...
    const result=evaluateForward(signal,mountain);
...
function study(){
  for(const row of latest.signals||[]){
    ...
    // The worker's shadow is the executable simulation: T decision, T+1 entry, T+2 exit.
    if(!settled.has(row.signalId)&&['WON','LOST'].includes(row.shadow?.outcome)){
      ...
      const p=summary(state.passes);state.lastLesson=`FORWARD CLASSROOM: executable T+1→T+2 PRE-ARM set ${p.trades}T · ${p.winRate.toFixed(1)}% · ${money(p.pnl)}.`;settled.add(row.signalId);captured.delete(row.signalId);save();
    }
  }
}
...
function snapshot(){return{version:'Forward Timing v2',offsetTicks:1,preArmConfirm:PREARM_CONFIRM,healthy:prearmHealthy(),latencyHeld:latencyHeld(),latencyHoldUntil:state.latencyHoldUntil,lastAckMs:state.lastAckMs,passes:summary(state.passes),rejects:summary(state.rejects),recent:recent(state.passes.recent,20),medianBuyAckMs:median(state.paidAckMs),medianEntrySlip:median(state.paidSlips),lastDecision:state.lastDecision,lastLesson:state.lastLesson,mountain}}
