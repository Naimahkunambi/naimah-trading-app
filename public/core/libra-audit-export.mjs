...
function exportAudit(){
  const rows=window.LIBRA?.getSignals?.()||[];
  const headers=[
    ...
    'regime','shadow_sani_outcome','shadow_sani_profit','shadow_libra_action','shadow_libra_direction','shadow_libra_outcome','shadow_libra_profit',
  ...
  const body=rows.map(row=>{
    const trades=row.actualTrades||[],trade=trades.find(t=>['WON','LOST'].includes(t.outcome))||trades[0]||{};
    return[
      ...
      row.libra?.regime,row.shadowResult?.sani?.outcome,row.shadowResult?.sani?.profit,row.shadowLibra?.action,row.shadowLibra?.direction,row.shadowResult?.libra?.outcome,row.shadowResult?.libra?.profit,
...
function bind(){
  ...
  if(!button||button.dataset.auditExport==='1')return;
  button.dataset.auditExport='1';
